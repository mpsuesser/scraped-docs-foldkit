...
Foldkit gained [server rendering](https://foldkit.dev/blog/foldkit-has-server-rendering) two weeks ago. It worked, but building a server-rendered application still happened across three commands: build the browser bundle, build the server bundle, then prerender the configured routes.
...
This is a breaking change, and `@foldkit/ui` 0.155.0 now requires `foldkit` 0.155.0 because its Mount definitions use the new shape. The [Mount docs](https://foldkit.dev/core/mount) have the complete migration.
...
The [HoverIntent docs](https://foldkit.dev/ui/hover-intent) have current examples and API details.
