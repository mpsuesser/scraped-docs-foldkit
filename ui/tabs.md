...
Present on the active tab button and its panel.

`data-disabled`

Present on disabled tab buttons.
...
Move to the next tab. In Automatic mode, also selects it.
...
Unique ID for the tabs instance.
...
The tabs state from your parent Model.

`toParentMessage`

`(childMessage: Tabs.Message) => ParentMessage`
...
The list of tab values, in display order. When the tabs component is declared via
...
The active tab, owned by the parent Model and passed back in on each render.
...
marker, the active panel, and

`RenderInfo.activeIndex`

all derive from it. Update it by folding the
...
Controls arrow key direction and

`aria-orientation`

. Horizontal uses left/right, vertical uses up/down.
...
Spread onto the tab list container. Includes
...
`ReadonlyArray<TabInfo<Value>>`

—

One entry per tab in

`ViewConfig.tabs`

, in the same order. See TabInfo below.
...
The currently-active tab index. Convenient when the consumer wants to render only the active panel (vs all panels with
...
The tab value. Typed as your

`Value`

union when the tabs component is declared via
...
Whether this tab is disabled via
...
Spread onto the tab button element. Includes
...
, the click handler, and the keyboard handler.
...
Spread onto the tab panel element. Includes
...
Emitted when a tab is committed via click or keyboard. Carries both the tab’s value (typed as your
...
) and its index. Pattern-match the third tuple element of
