...
- **Release mail:** the [newsletter](https://foldkit.dev/newsletter) sends new releases, patterns, and the occasional deep dive.
...
An agent integrating with Foldkit does not need to contact anyone. The [Content API](https://foldkit.dev/api) describes the site's machine-readable surface, [llms.txt](https://foldkit.dev/llms.txt) indexes every page, and the [DevTools MCP server](https://foldkit.dev/ai/mcp) connects an agent to a running Foldkit application.
